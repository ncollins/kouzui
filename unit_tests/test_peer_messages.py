...
def test_messages_with_no_payload() -> None:
    for message_type in (Choke, Unchoke, Interested, NotInterested, KeepAlive):
        assert message_type() == parse_message(message_type()._to_payload())
        assert message_type(), b"" == parse_message_with_length(message_type().to_bytes())


@given(ST_4_BYTE_UINT)
def test_have(i: int) -> None:
    msg = Have(piece_index=i)
    assert msg == parse_message(msg._to_payload())
    assert msg, b"" == parse_message_with_length(msg.to_bytes())

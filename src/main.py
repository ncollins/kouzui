...
def test(test_dir: Path, torrent_file: Path, number_of_clients: int, *, cfg: config.Config) -> None:
    ...
    for i in range(number_of_clients):
        ...
        p = mp.Process(
            target=run,
            args=("INFO", torrent_file, 50000 + i, client_dir, False),
            kwargs={"cfg": cfg},
        )
...
def run_command(
    ...
    run(log_level, torrent_file, listening_port, download_dir, auto_shutdown, cfg=cfg)
...
def parse_torrent_file_command(
    ...
    print(f"tracker_address: {t.tracker_address.decode()}")
    print(f"tracker_port: {t.tracker_port}")
    print(f"tracker_path: {t.tracker_path.decode()}")
